...
def search_flights(origin: str, destination: str) -> str:
    ...
    """
    # Thử tìm theo chiều thuận
    flights = FLIGHTS_DB.get((origin, destination))
    
    # Nếu không thấy, thử tìm theo chiều ngược lại (tra ngược)
    if not flights:
        flights = FLIGHTS_DB.get((destination, origin))
        if flights:
            origin, destination = destination, origin # Update label để hiển thị đúng
    ...
    for f in flights:
        ...
        result += f"- {f['airline']} ({f['class']}): {f['departure']} -> {f['arrival']} | Giá: {price_fmt}đ\n"
    return result
...
def search_hotels(city: str, max_price_per_night: int = 99999999) -> str:
    ...
    for h in filtered_hotels:
        ...
        result += f"- {h['name']} ({h['stars']}*): {price_fmt}đ/đêm | Khu vực: {h['area']} | Rating: {h['rating']}\n"
    return result
...
def calculate_budget(total_budget: int, expenses: str) -> str:
    ...
    try:
        ...
        for name, amount in expense_dict.items():
            result += f"- {name.replace('_', ' ').capitalize()}: {amount:,}đ\n".replace(",", ".")
        ...
        if remaining < 0:
            result += f"Cảnh báo: Vượt ngân sách {abs(remaining):,}đ! Cần điều chỉnh.".replace(",", ".")
            
        return result
    except Exception:
        return "Lỗi: Định dạng chuỗi expenses sai. Vui lòng sử dụng 'tên:số_tiền,tên:số_tiền'."
